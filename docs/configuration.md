...
- 推送周期：支持每天或每周定点推送。
- 推送时间：格式为 `HH:MM`，例如每天 `11:00`，或每周一 `11:00`。
- 周末推送：关闭时，系统不会在周六、周日发送低余额提醒；每天推送会顺延到下一个工作日，每周选择周末时不产生推送排程。
- 标题模板、内容模板：支持 `{name}`、`{balance}`、`{threshold}`、`{department}`、`{card_no}`、`{wanoa_pin}`、`{now}` 等变量。
- 测试条件：按当前页面表单内容预览命中用户、预计推送时间和消息内容，不会实际发送消息，也不要求先保存设置。
- 推送日志：实际发送成功或失败会写入“日志中心 / 操作记录”，操作类型为 `low_balance_alert_push`。
